...
def get_reportes_router(db: Database) -> Router:
    ...
    async def cmd_resumen(event: types.Message | types.CallbackQuery):
        ...
        try:
            ...
            if len(fincas) == 1:
                await mostrar_resumen(send, fincas[0]["id"], fincas[0]["nombre"])
                return
            ...
            await send("❌ *Error al generar resumen.*", parse_mode="Markdown")
    ...
    async def seleccionar_finca_resumen(callback: types.CallbackQuery):
        ...
        await mostrar_resumen(callback.message.edit_text, finca_id, finca["nombre"])

    async def mostrar_resumen(send_func, finca_id: int, finca_nombre: str):
        ...
        texto = (
            ...
            f"📐 *Área total:* {area:.2f} ha\n\n"
            f"💰 *Ingresos Totales:* ${total_ingresos:,.0f}\n"
            f"📉 *Egresos Totales:* ${total_egresos:,.0f}\n"
        )

        if margen >= 0:
            texto += f"✅ *Margen:* ${margen:,.0f}\n"
        else:
            texto += f"❌ *Margen:* -${abs(margen):,.0f} (pérdida)\n"

        texto += f"📊 *Costo por hectárea:* ${costo_ha:,.0f}\n\n"

        # Egresos por categoría
        if resumen["egresos_por_categoria"]:
            texto += "*Egresos por categoría:*\n"
        ...
        for i, parte in enumerate(partes):
            if i == len(partes) - 1:
                await send_func(parte, parse_mode="Markdown", reply_markup=keyboard)
            else:
                await send_func(parte, parse_mode="Markdown")
    ...
    async def cmd_generar_excel(callback: types.CallbackQuery):
        ...
        try:
            await callback.message.edit_text(
                "⏳ *Generando Excel...* Esto puede tomar unos segundos.",
                parse_mode="Markdown",
            )
            ...
            with open(output_path, "rb") as f:
                await callback.message.answer_document(
                    ...
                    caption="📊 *Excel de Costos de Producción generado* ☕\n\n"
            ...
            await callback.message.edit_text(
                "❌ *Error:* El template Excel no está disponible.\n\n"
            ...
            await callback.message.edit_text(
                "❌ *Error al generar el Excel.* Intenta de nuevo más tarde.",
                parse_mode="Markdown",
            )
